    ...
    switch (record.type) {
      case ReplicationLogRecord::kMemtableWrite: {
        ...
        if (versions_->LastSequence() + 1 !=
          ...
          return Status::Corruption("Gap in sequence numbers");
        }
        ...
        for (auto& e : edits) {
          if (!e.HasManifestUpdateSequence()) {
            return Status::InvalidArgument(
                "Manifest write doesn't have an update sequence number");
          }
          ...
          if (e.GetManifestUpdateSequence() != current_update_sequence) {
            return Status::Corruption("Gap in Manifest Update Sequence");
          }
